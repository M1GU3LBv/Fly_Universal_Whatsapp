...
client.on("qr", async (qr) => {
  console.log(
    `[${moment().tz(config.timezone).format("HH:mm:ss")}] Escanea el codigo`
  );
...
app.post("/send-message", async (req, res) => {
  ...
  if (!to || !message) {
    return res
      .status(400)
      .json({ error: 'Faltan los campos "to" o "message".' });
  }
...
app.get("/logout", (req, res) => {
  client.logout();
  console.log(
    `[${moment().tz(config.timezone).format("HH:mm:ss")}] Cerrando sesión..`
  );
  res.redirect("/qr");
});
...
client.on("message", async (message) => {
  if (message.body.startsWith(`${config.prefix}weather`)) {
    ...
    if (url) {
      try {
        const response = await fetch(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${url}.json?access_token=${config.mapboxToken}`
        );
        const data = await response.json();
    ...
    if (names.length !== 2) {
      client.sendMessage(
        message.from,
        "Por favor, proporciona exactamente dos nombres."
      );
      return;
    }
    ...
    if (isGroup) {
      ...
      const sender = chat.participants.find(
        (participant) => participant.id._serialized === message.author
      );
    ...
    const file = fs.readFileSync(
      "./static/audio/x2mate.com - levantate de pie muchacho (128 kbps).mp3"
    );
    ...
    const file = fs.readFileSync(
      "./static/audio/vamos gg brianeitor [TubeRipper.com].mp3"
    );
